...
JSPLib.menu.renderTab = function () {
    return `<li><a href="#${this.program_selector}">${JSPLib.program_name}</a></li>`;
};
...
JSPLib.menu.mainSettingsClick = function (override) {
    const printer = JSPLib.debug.getFunctionPrint('menu.mainSettingsClick');
    if (override || !JSPLib.utility.isNamespaceBound(this.settings_selector, 'click', 'jsplib')) {
        printer.debuglogLevel("Installing main setting click", JSPLib.debug.DEBUG);
...
JSPLib.menu.otherSettingsClicks = function (override) {
    const printer = JSPLib.debug.getFunctionPrint('menu.otherSettingsClicks');
    if (override || !JSPLib.utility.isNamespaceBound(this.other_selectors, 'click', 'jsplib')) {
        printer.debuglogLevel("Installing other setting click", JSPLib.debug.DEBUG);
...
JSPLib.menu.installSettingsMenu = function () {
    const printer = JSPLib.debug.getFunctionPrint('menu.installSettingsMenu');
    ...
    if (jQuery("#userscript-settings-menu").length === 0) {
        ...
        if (version < this.version) {
            printer.debuglogLevel("Lower menu version installed", version || 0, ", installing version", this.version, JSPLib.debug.INFO);
            ...
            jQuery('#userscript-settings-menu').each((_, entry) => jQuery(entry).remove());
            ...
            jQuery('style').filter((_, entry) => entry.textContent.match(/#userscript-menu-link/)).remove();
            jQuery('style').filter((_, entry) => entry.textContent.match(/\/\*\*\*JQUERY-UI\*\*\*\//)).remove();
...
JSPLib.menu.initializeSettingsMenu = function (RenderSettingsMenu, menu_CSS) {
    const printer = JSPLib.debug.getFunctionPrint('menu.initializeSettingsMenu');
    if (!RenderSettingsMenu) return;
    ...
    if (typeof JSPLib._jQuery.fn.tabs !== 'function') {
        if (!JSPLib.utility.getPublicData(document.body).jsplibInstalling) {
            if (JSPLib.network) {
                printer.debuglogLevel("Installing script with network.", JSPLib.debug.INFO);
                install_promise = JSPLib.network.getScript(this.tab_widget_url);
            } else {
                printer.debuglogLevel("Installing script with DOM.", JSPLib.debug.INFO);
                JSPLib.utility.installScriptDOM(this.tab_widget_url);
            }
            JSPLib._jQuery(document.body).data('jsplib-installing', true);
        } else {
            printer.debuglogLevel("Script is installing.", JSPLib.debug.INFO);
        }
    } else {
        printer.debuglogLevel("Script already installed.", JSPLib.debug.INFO);
    ...
    if (install_promise === null) {
        install_promise = new Promise((resolve) => {
            let timer = setInterval(() => {
                if (typeof JSPLib._jQuery.fn.tabs === 'function') {
                    printer.debuglogLevel("Script detected.", JSPLib.debug.VERBOSE);
                    clearInterval(timer);
                    resolve(null);
                }
            }, 100);
    ...
    Promise.all([install_promise, css_promise]).then(() => {
        printer.debuglogLevel("Script is installed.", JSPLib.debug.DEBUG);
...
JSPLib.menu.renderTextinput = function (setting_name, length = 20, is_control = false) {
    ...
    let textinput_key = `${JSPLib.program_shortcut}-${menu_type}-${setting_key}`;
    let submit_control = "";
    ...
    return `
<div class="${JSPLib.program_shortcut}-textinput jsplib-textinput jsplib-menu-item" data-setting="${setting_name}">
    <h4>${display_name}</h4>
    <div>
        <input type="text" class="${JSPLib.program_shortcut}-${menu_type} jsplib-${menu_type}" name="${textinput_key}" id="${textinput_key}" value="${value}" size="${length}" autocomplete="off" data-parent="2">
        ${submit_control}
        ${hint_html}
    </div>
</div>`;
...
JSPLib.menu.renderCheckbox = function (setting_name, is_control = false) {
    ...
    return `
<div class="${JSPLib.program_shortcut}-checkbox jsplib-checkbox jsplib-menu-item" data-setting="${setting_name}">
    <h4>${display_name}</h4>
    <div>
        <input type="checkbox" ${checked} class="${JSPLib.program_shortcut}-${menu_type} jsplib-${menu_type}" name="${JSPLib.program_shortcut}-enable-${setting_key}" id="${JSPLib.program_shortcut}-enable-${setting_key}"  data-parent="2">
        ${hint_html}
    </div>
</div>`;
};
...
JSPLib.menu.renderSortlist = function (setting_name) {
    ...
    sort_list.forEach((sortitem) => {
        ...
        let sortitem_key = `${JSPLib.program_shortcut}-enable-${setting_key}-${JSPLib.utility.kebabCase(sortitem)}`;
        html += `
<li class="ui-state-default">
    <input type="hidden" class="${JSPLib.program_shortcut}-setting jsplib-setting" name="${sortitem_key}" id="${sortitem_key}" data-sort="${sortitem}" data-parent="4">
    <div>
    ...
    return `
<div class="${JSPLib.program_shortcut}-sortlist jsplib-sortlist jsplib-menu-item" data-setting="${setting_name}">
    <h4>${display_name}</h4>
    <div>
        <ul>
            ${html}
...
JSPLib.menu.renderInputSelectors = function (setting_name, type, is_control = false, has_submit = false) {
    ...
    let menu_type = (is_control ? 'control' : 'setting');
    let selection_name = `${JSPLib.program_shortcut}-${menu_type}-${setting_key}`;
    ...
    config[setting_name].allitems.forEach((selector) => {
        ...
        let selection_key = `${JSPLib.program_shortcut}-select-${setting_key}-${selector}`;
        html += `
            <label for="${selection_key}">${display_selection}</label>
            <input type="${type}" ${checked} class="${JSPLib.program_shortcut}-${menu_type} jsplib-${menu_type}" name="${selection_name}" id="${selection_key}" data-selector="${selector}" data-parent="2">`;
    });
    return `
<div class="${JSPLib.program_shortcut}-selectors jsplib-selectors jsplib-menu-item" data-setting="${setting_name}">
    <h4>${display_name}</h4>
    <div>
        ${html}
        ${submit_control}
...
JSPLib.menu.renderDomainSelectors = function () {
    let selection_name = `${JSPLib.program_shortcut}-domain-selector`;
    ...
    this.domains.forEach((selector) => {
        let checked = (enabled_selectors.includes(selector) ? "checked" : "");
        let display_selection = JSPLib.utility.displayCase(selector);
        let selection_key = `${JSPLib.program_shortcut}-select-domain-${selector}`;
        html += `
            <label for="${selection_key}">${display_selection}</label>
            <input type="checkbox" ${checked} class="${JSPLib.program_shortcut}-global jsplib-global" name="${selection_name}" id="${selection_key}" data-selector="${selector}" data-parent="2">`;
    });
    return `
<div class="${JSPLib.program_shortcut}-selectors jsplib-selectors jsplib-menu-item" data-setting="domain_selector">
...
JSPLib.menu.renderKeyselect = function (setting_name, is_control = false) {
    ...
    return `
<div class="${JSPLib.program_shortcut}-options jsplib-options jsplib-menu-item" data-setting="${setting_name}">
    <h4>${display_name}</h4>
    <div>
        <select name="${selection_name}" id="${selection_name}" class="${JSPLib.program_shortcut}-${menu_type} jsplib-${menu_type}" data-parent="2">;
            ${html}
        </select>
        ${hint_html}
    </div>
...
JSPLib.menu.renderLinkclick = function (setting_name) {
    ...
    let hint_html = this.renderSettingHint("inline", config[setting_name].hint);
    return `
<div class="${JSPLib.program_shortcut}-linkclick jsplib-linkclick jsplib-menu-item">
    <h4>${display_name}</h4>
    <div>
        <b>
            <span class="${JSPLib.program_shortcut}-control jsplib-control">
                [ <a id="${JSPLib.program_shortcut}-control-${setting_key}">${link_text}</a> ]
            </span>
        </b>
        &emsp;
        ${hint_html}
...
JSPLib.menu.renderDataSourceSections = function () {
    ...
    let section_class = `${JSPLib.program_shortcut}-section-data-source`;
    let html = "";
    allitems.forEach((source) => {
        let style = (source !== value ? `style="display:none"` : "");
        let source_key = `${JSPLib.program_shortcut}-section-${JSPLib.utility.kebabCase(source)}`;
...
JSPLib.menu.renderControlButton = function (setting_key, button_name, parent_level) {
    let button_key = `${JSPLib.program_shortcut}-${setting_key}-${button_name}`;
    let display_name = JSPLib.utility.displayCase(button_name);
    return `<input type="button" class="jsplib-control ${JSPLib.program_shortcut}-control" name="${button_key}" id="${button_key}" value="${display_name}" data-parent="${parent_level}">`;
};

JSPLib.menu.renderSettingHint = function (type, hint) {
    return `<span class="${JSPLib.program_shortcut}-menu-tooltip jsplib-${type}-tooltip">${hint}</span>`;
};

JSPLib.menu.renderExpandable = function (header, content) {
    return `
    <div class="${JSPLib.program_shortcut}-expandable jsplib-expandable jsplib-prose">
...
JSPLib.menu.renderCacheControls = function () {
    return `
<div id="${JSPLib.program_shortcut}-cache-controls" class="jsplib-controls-grouping">
    <div id="${JSPLib.program_shortcut}-cache-controls-message" class="prose">
...
JSPLib.menu.renderCacheInfoTable = function () {
    return `<div id="${JSPLib.program_shortcut}-cache-info-table" style="display:none"></div>`;
};

JSPLib.menu.renderLocalStorageSource = function () {
    return `<input id="${JSPLib.program_shortcut}-control-data-source" type="hidden" value="local_storage">`;
};

JSPLib.menu.renderCacheEditor = function (has_cache_data) {
    let message = (has_cache_data ? `<p>See the <b><a href="#${JSPLib.program_shortcut}-cache-controls-message">Cache Data</a></b> details for the list of all cache data and what they do.</p>` : "");
    return `
<div id="${JSPLib.program_shortcut}-cache-editor">
    <div id="${JSPLib.program_shortcut}-cache-editor-message" class="prose">
        <h4>Cache editor</h4>
        ${message}
    </div>
    <div id="${JSPLib.program_shortcut}-cache-editor-controls"></div>
    <div id="${JSPLib.program_shortcut}-cache-editor-errors" class="jsplib-cache-editor-errors"></div>
    <div id="${JSPLib.program_shortcut}-cache-viewer" class="jsplib-cache-viewer">
        <textarea></textarea>
    </div>
</div>`;
};
...
JSPLib.menu.renderMenuSection = function (value, type) {
    ...
    return `
<div id="${JSPLib.program_shortcut}-${section_key}-${type}" class="jsplib-${type}-grouping">
    <div id="${JSPLib.program_shortcut}-${section_key}-${type}-message" class="prose">
...
JSPLib.menu.renderMenuFramework = function (menu_config) {
    ...
    let topic_message = (menu_config.topic_id ? `<p>Check the forum for the latest on information and updates (<a class="dtext-link dtext-id-link dtext-forum-topic-id-link" href="https://danbooru.donmai.us/forum_topics/${menu_config.topic_id}">topic #${menu_config.topic_id}</a>).</p>` : "");
    ...
    return `
<div id="${JSPLib.program_shortcut}-script-message" class="prose">
    <h2>${JSPLib.program_name}</h2>
    ${topic_message}
    ${wiki_message}
</div>
<div id="${JSPLib.program_shortcut}-console" class="jsplib-console">
    <div id="${JSPLib.program_shortcut}-settings" class="jsplib-outer-menu">
        ${settings_html}
        <div id="${JSPLib.program_shortcut}-settings-buttons" class="jsplib-settings-buttons">
            <input type="button" id="${JSPLib.program_shortcut}-commit" class="jsplib-commit" value="Save">
            <input type="button" id="${JSPLib.program_shortcut}-resetall" class="jsplib-resetall" value="Factory Reset">
        </div>
    </div>
    <div id="${JSPLib.program_shortcut}-controls" class="jsplib-outer-menu">
        ${control_html}
    </div>
</div>`;
};
...
JSPLib.menu.getProgramValues = function (setting_name, is_control = false) {
    ...
    let item = (!is_control ? JSPLib.program_data.user_settings[setting_name] : config[setting_name].value);
    return [config, setting_key, display_name, item];
};

JSPLib.menu.isSettingEnabled = function (setting_name, selector) {
    return Boolean(JSPLib.program_data.user_settings[setting_name]) && JSPLib.program_data.user_settings[setting_name].includes(selector);
};

JSPLib.menu.getCheckboxRadioSelected = function (selector) {
    return JSPLib._jQuery(selector).map((_, input) => (input.checked ? JSPLib._jQuery(input).data('selector') : undefined)).toArray();
};

JSPLib.menu.engageUI = function (is_checkbox = false, is_sortable = false) {
    if (is_checkbox) {
        JSPLib._jQuery(`.${JSPLib.program_shortcut}-selectors input`).checkboxradio();
    }
    if (is_sortable) {
        JSPLib._jQuery(`.${JSPLib.program_shortcut}-sortlist ul`).sortable();
    }
...
JSPLib.menu.preloadScript = function (program_value, {render_menu_func = null, run_on_settings = false, default_data = {}, reset_data = {}, initialize_func = null, broadcast_func = null, menu_css = null, program_css = null, light_css = null, dark_css = null, danbooru_userscript = true} = {}) {
    const printer = JSPLib.debug.getFunctionPrint('menu.preloadScript');
    ...
    if (danbooru_userscript) {
        ...
        if (!this.isScriptEnabled()) {
            printer.debuglogLevel("Script is disabled on", JSPLib._window.location.hostname, JSPLib.debug.INFO);
            return false;
        }
        danbooru_dataset = {
    ...
    if (typeof broadcast_func == 'function') {
        program_value.channel = JSPLib.utility.createBroadcastChannel(JSPLib.program_name, broadcast_func);
    }
    if (program_css) {
        JSPLib.utility.setCSSStyle(program_css, 'program');
    }
    if (light_css) {
        JSPLib.utility.setCSSStyle(JSPLib.utility.renderColorScheme(light_css, 'light'), 'light');
    }
    if (dark_css) {
        JSPLib.utility.setCSSStyle(JSPLib.utility.renderColorScheme(dark_css, 'dark'), 'dark');
    }
...
JSPLib.menu.getEnabledDomains = function () {
    const printer = JSPLib.debug.getFunctionPrint('menu.getEnabledDomains');
    if (this._current_domains !== null) {
        return this._current_domains;
    }
    let domains = JSPLib.utility.readCookie(JSPLib.program_name);
    if (!domains) {
        return this.domains;
    }
    if (domains === 'none') {
    ...
    if (this._current_domains.length === 0) {
        printer.debuglogLevel("Invalid cookie found!", JSPLib.debug.WARNING);
        JSPLib.utility.eraseCookie(JSPLib.program_name, 'shiteitte.net');
        this._current_domains = JSPLib.utility.dataCopy(this.domains);
    } else if (this._current_domains.length !== cookie_domains.length) {
        printer.debuglogLevel("Invalid domains found on cookie!", JSPLib.debug.WARNING);
        JSPLib.utility.createCookie(JSPLib.program_name, this._current_domains.join(','), null, 'shiteitte.net');
    }
    return this._current_domains;
};
...
JSPLib.menu.loadUserSettings = function () {
    const printer = JSPLib.debug.getFunctionPrint('menu.loadUserSettings');
    let config = this.settings_config;
    let settings = JSPLib.storage.getLocalData(`${JSPLib.program_shortcut}-user-settings`, {default_val: {}});
    let dirty = false;
    if (Array.isArray(this.settings_migrations)) {
        this.settings_migrations.forEach((migration) => {
            if (config[migration.to].validate((settings[migration.from]))) {
                printer.debuglogLevel("Migrating setting: ", migration.from, "->", migration.to, JSPLib.debug.INFO);
    ...
    if (!JSPLib.utility.isHash(settings)) {
        printer.debugwarnLevel("User settings are not a hash!", JSPLib.debug.ERROR);
        settings = {};
    }
    let errors = this.validateUserSettings(settings);
    if (errors.length) {
        printer.debuglogLevel("Errors found:\n", errors.join('\n'), JSPLib.debug.WARNING);
        dirty = true;
    }
    if (dirty) {
        printer.debuglogLevel("Saving updated changes to user settings!", JSPLib.debug.INFO);
        JSPLib.storage.setLocalData(`${JSPLib.program_shortcut}-user-settings`, settings);
    }
    printer.debuglogLevel("Returning settings:", settings, JSPLib.debug.DEBUG);
    return settings;
};

JSPLib.menu.validateUserSettings = function (settings) {
    const printer = JSPLib.debug.getFunctionPrint('menu.validateUserSettings');
    let error_messages = [];
    ...
    for (let setting in config) {
        if (!(setting in settings) || !config[setting].validate(settings[setting])) {
            ...
            printer.debuglogLevel(`${message}:`, setting, old_setting, "->", settings[setting], JSPLib.debug.WARNING);
        }
    }
    let valid_settings = Object.keys(config);
    for (let setting in settings) {
        if (!valid_settings.includes(setting)) {
            printer.debuglogLevel("Deleting invalid setting:", setting, settings[setting], JSPLib.debug.WARNING);
...
JSPLib.menu.validateCheckboxRadio = function (data, type, allitems) {
    return Array.isArray(data)
        && data.every((val) => JSPLib.utility.isString(val))
...
JSPLib.menu.validateNumber = function (data, is_integer, min, max) {
    const validator = (is_integer ? Number.isInteger : JSPLib.utility.isNumber);
    min = min || -Infinity;
    max = max || Infinity;
...
JSPLib.menu.updateUserSettings = function () {
    let settings = JSPLib.program_data.user_settings;
    JSPLib._jQuery(`#${JSPLib.program_shortcut}-settings .${JSPLib.program_shortcut}-setting[id]`).each((_, entry) => {
...
JSPLib.menu.updateGlobalSettings = function () {
    ...
    JSPLib._jQuery(`#${JSPLib.program_shortcut}-settings .${JSPLib.program_shortcut}-global[id]`).each((_, entry) => {
        let $input = JSPLib._jQuery(entry);
        let parent_level = $input.data('parent');
        let container = JSPLib.utility.getNthParent(entry, parent_level);
        let setting_name = JSPLib._jQuery(container).data('setting');
...
JSPLib.menu.hasSettingChanged = function (setting_name) {
    return JSON.stringify(JSPLib.program_data.user_settings[setting_name]) !== JSON.stringify(JSPLib.program_data.old_settings[setting_name]);
};
...
JSPLib.menu.saveUserSettingsClick = function (local_callback = null) {
    let current_domains = this.getEnabledDomains();
    JSPLib._jQuery(`#${JSPLib.program_shortcut}-commit`).on(JSPLib.program_click, () => {
        let config = this.settings_config;
        let settings = JSPLib.program_data.user_settings;
        JSPLib.program_data.old_settings = JSPLib.utility.dataCopy(settings);
        let invalid_setting = JSPLib.menu._collectSettingsInputs(config, settings);
        JSPLib.storage.setLocalData(`${JSPLib.program_shortcut}-user-settings`, settings);
        this._channel.postMessage({type: "settings", program_shortcut: JSPLib.program_shortcut, from: JSPLib.UID.value, user_settings: settings});
        if (JSPLib._jQuery(`#${JSPLib.program_shortcut}-settings .${JSPLib.program_shortcut}-global`).length) {
            let selected_domains = this.getCheckboxRadioSelected(`#${JSPLib.program_shortcut}-settings .${JSPLib.program_shortcut}-global`);
            if (JSPLib.utility.arrayEquals(this.domains, selected_domains)) {
                ...
                JSPLib.utility.createCookie(JSPLib.program_name, 'none', 365, 'shiteitte.net');
            } else {
                JSPLib.utility.createCookie(JSPLib.program_name, selected_domains.join(','), 365, 'shiteitte.net');
            }
            ...
            if (enabled_domains.length || disabled_domains.length) {
                this._channel.postMessage({type: 'domain', program_shortcut: JSPLib.program_shortcut, from: JSPLib.UID.value, enabled_domains, disabled_domains, current_domains: selected_domains});
            }
            this._current_domains = selected_domains;
        }
        if (!invalid_setting) {
            JSPLib.notice.notice(`<b>${JSPLib.program_name}:</b> Settings updated!`);
        } else {
...
JSPLib.menu.resetUserSettingsClick = function (delete_keys = [], local_callback = null) {
    JSPLib._jQuery(`#${JSPLib.program_shortcut}-resetall`).on(JSPLib.program_click, () => {
        let config = this.settings_config;
        let settings = JSPLib.program_data.user_settings;
        JSPLib.program_data.old_settings = JSPLib.utility.dataCopy(settings);
        if (confirm(`This will reset all of ${JSPLib.program_name}'s settings.\n\nAre you sure?`)) {
            for (let setting in config) {
                settings[setting] = config[setting].reset;
            }
            JSPLib._jQuery(`#${JSPLib.program_shortcut}-settings .${JSPLib.program_shortcut}-setting[id]`).each((_, entry) => {
            ...
            Object.assign(JSPLib.program_data, JSPLib.utility.dataCopy(this.program_reset_data), {storage_keys: {local_storage: []}});
            JSPLib.storage.setLocalData(`${JSPLib.program_shortcut}-user-settings`, settings);
            this._channel.postMessage({type: 'reset', program_shortcut: JSPLib.program_shortcut, from: JSPLib.UID.value, user_settings: settings});
            JSPLib.notice.notice(`<b>${JSPLib.program_name}:</b> Settings reset to defaults!`);
        }
        if (typeof local_callback === 'function') {
            local_callback();
        }
...
JSPLib.menu.purgeCacheClick = function () {
    JSPLib._jQuery(`#${JSPLib.program_shortcut}-control-purge-cache`).on(JSPLib.program_click, (event) => {
        if (!this._purge_is_started && confirm(`This will delete all of ${JSPLib.program_name}'s cached data.\n\nAre you sure?`)) {
...
JSPLib.menu.cacheInfoClick = function () {
    JSPLib._jQuery(`#${JSPLib.program_shortcut}-control-cache-info`).on(JSPLib.program_click, (event) => {
        JSPLib.notice.notice("Calculating cache information...");
        JSPLib.storage.programCacheInfo(this.program_data_regex).then((cache_info) => {
            let html = `
...
`;
            JSPLib._jQuery(`#${JSPLib.program_shortcut}-cache-info-table`).html(html).show();
        });
        event.preventDefault();
    });
};
...
JSPLib.menu.expandableClick = function () {
    JSPLib._jQuery(`.${JSPLib.program_shortcut}-expandable .jsplib-expandable-button`).on(JSPLib.program_click, (event) => {
...
JSPLib.menu.loadStorageKeys = async function () {
    ...
    let storage_keys = JSPLib.program_data.storage_keys = {};
...
JSPLib.menu.getCacheDatakey = function () {
    JSPLib.program_data.data_source = JSPLib._jQuery(`#${JSPLib.program_shortcut}-control-data-source`).val();
    JSPLib.program_data.data_type = JSPLib._jQuery(`#${JSPLib.program_shortcut}-control-data-type`).val();
    let data_key = JSPLib.program_data.data_value = JSPLib._jQuery(`#${JSPLib.program_shortcut}-control-data-name`).val().trim();
    if (JSPLib.program_data.data_source === "local_storage") {
        JSPLib.program_data.raw_data = JSPLib._jQuery(`#${JSPLib.program_shortcut}-enable-raw-data`).prop('checked');
        data_key = JSPLib.program_shortcut + '-' + JSPLib.program_data.data_value;
    } else if (JSPLib.program_data.data_type !== "custom") {
        if (typeof this.program_data_key === "function") {
            data_key = this.program_data_key(JSPLib.program_data.data_type, JSPLib.program_data.data_value);
...
JSPLib.menu.saveLocalData = function (key, data, validator, localupdater) {
    if (validator(key, data)) {
        ...
        if (key === `${JSPLib.program_shortcut}-user-settings`) {
            JSPLib.program_data.user_settings = data;
            this.updateUserSettings();
            this._channel.postMessage({type: "settings", program_shortcut: JSPLib.program_shortcut, from: JSPLib.UID.value, user_settings: data});
...
JSPLib.menu.adjust_data_name = function (disable = true) {
    let name_selector = `#${JSPLib.program_shortcut}-control-data-name`;
...
JSPLib.menu.dataSourceChange = function () {
    JSPLib._jQuery(`#${JSPLib.program_shortcut}-control-data-source`).on(JSPLib.program_change, () => {
        let data_source = JSPLib._jQuery(`#${JSPLib.program_shortcut}-control-data-source`).val();
        JSPLib._jQuery(`.${JSPLib.program_shortcut}-section-data-source`).hide();
        let shown_key = `#${JSPLib.program_shortcut}-section-${JSPLib.utility.kebabCase(data_source)}`;
...
JSPLib.menu.rawDataChange = function () {
    JSPLib._jQuery(`#${JSPLib.program_shortcut}-enable-raw-data`).on(JSPLib.program_change, (event) => {
...
JSPLib.menu.getCacheClick = function (localvalidator) {
    JSPLib._jQuery(`#${JSPLib.program_shortcut}-data-name-get`).on(JSPLib.program_click, () => {
        let storage_key = this.getCacheDatakey();
        if (JSPLib.program_data.data_source === "local_storage") {
            let data = {};
            if (JSPLib.program_data.raw_data) {
            ...
            JSPLib._jQuery(`#${JSPLib.program_shortcut}-cache-viewer textarea`).val(JSON.stringify(data, null, 2));
        } else {
...
JSPLib.menu.saveCacheClick = function (localvalidator, indexvalidator, localupdater) {
    JSPLib._jQuery(`#${JSPLib.program_shortcut}-data-name-save`).on(JSPLib.program_click, () => {
        var data;
        try {
            data = JSON.parse(JSPLib._jQuery(`#${JSPLib.program_shortcut}-cache-viewer textarea`).val());
        } catch (error) {
            JSPLib.notice.error("Invalid JSON data! Unable to save.");
            return;
        }
        JSPLib.validate.dom_output = `#${JSPLib.program_shortcut}-cache-editor-errors`;
        let storage_key = this.getCacheDatakey();
        if (JSPLib.program_data.data_source === "local_storage") {
            if (JSPLib.program_data.raw_data) {
                let error_messages = [];
                let $cache_errors = JSPLib._jQuery(`#${JSPLib.program_shortcut}-cache-editor-errors`);
                for (let key in data) {
                    let data_key = JSPLib.program_shortcut + '-' + key;
...
JSPLib.menu.deleteCacheClick = function () {
    JSPLib._jQuery(`#${JSPLib.program_shortcut}-data-name-delete`).on(JSPLib.program_click, () => {
        let storage_key = this.getCacheDatakey();
        if (JSPLib.program_data.data_source === "local_storage") {
...
JSPLib.menu.listCacheClick = function () {
    JSPLib._jQuery(`#${JSPLib.program_shortcut}-data-name-list`).on(JSPLib.program_click, () => {
        ...
        this._storage_keys_promise.then(() => {
            JSPLib._jQuery(`#${JSPLib.program_shortcut}-cache-viewer textarea`).val(JSON.stringify(JSPLib.program_data.storage_keys[JSPLib.program_data.data_source], null, 2));
        });
    });
};

JSPLib.menu.refreshCacheClick = function () {
    JSPLib._jQuery(`#${JSPLib.program_shortcut}-data-name-refresh`).on(JSPLib.program_click, () => {
...
JSPLib.menu.cacheSource = function () {
    ...
    return function (_, resp) {
        ...
        if ((JSPLib.program_data.data_source === "indexed_db" && JSPLib.program_data.data_value.length === 0) || !context._storage_keys_loaded) {
            resp([]);
            return;
        }
        let source_keys = JSPLib.program_data.storage_keys[JSPLib.program_data.data_source];
        ...
        if (JSPLib.program_data.data_source === 'local_storage') {
            transformed_keys = context._keyToNameTransform(transformed_keys, JSPLib.program_shortcut);
        } else if (JSPLib.program_data.data_type !== "custom") {
            let program_keys = transformed_keys.filter((key) => key.match(JSPLib.program_regex));
            let program_names = context._keyToNameTransform(program_keys, JSPLib.program_shortcut);
...
JSPLib.menu.cacheAutocomplete = function () {
    let $control_data = JSPLib._jQuery(`#${JSPLib.program_shortcut}-control-data-name`);
...
JSPLib.menu._collectSettingsInputs = function (config, settings) {
    let invalid_setting = false;
    let temp_selectors = {};
    let settings_inputs = JSPLib._jQuery(`#${JSPLib.program_shortcut}-settings .${JSPLib.program_shortcut}-setting[id]`);
...
JSPLib.menu._broadcastRX = function () {
    ...
    return function (event) {
        if (event.data.program_shortcut !== JSPLib.program_shortcut) {
            return;
        }
...
JSPLib.menu._updateSettingsFromBroadcast = function (new_settings) {
    JSPLib.program_data.old_settings = JSPLib.program_data.user_settings;
    JSPLib.program_data.user_settings = new_settings;
    if (this._isSettingMenu()) {
        this.updateUserSettings();
    }
};
...
Object.defineProperty(JSPLib.menu, 'program_selector', {get() {return JSPLib.utility.kebabCase(JSPLib.program_name);}});
