...
Load.programInitialize = function (entry_func, {function_name = null, required_variables = [], required_selectors = [], optional_selectors = [], max_retries = DEFAULT_MAX_RETRIES, timer_interval = DEFAULT_TIMER_INTERVAL, load_when_hidden = true} = {}) {
    ...
    if (JSPLib.name && function_name === null) {
        if (JSPLib._window_jsp.program[JSPLib.name]) {
            printer.warn(`${JSPLib.name} already loaded.`);
            return;
        }
...
Load.isScriptEnabled = function () {
    if (SCRIPT_ENABLED === null) {
        ...
        if (!domains) {
            SCRIPT_ENABLED = JSPLib.domains.includes(JSPLib._current_subdomain);
...
Load.preloadScript = function ({broadcast_func = null, program_css = null, light_css = null, dark_css = null, danbooru_userscript = true, run_on_settings = false} = {}) {
    ...
    if (danbooru_userscript) {
        ...
        if (!run_on_settings && document.body.dataset.controller === "users" && document.body.dataset.action === "edit") return;
        danbooru_dataset = {
...
Load.validateUserSettings = function (settings) {
...
function _loadUserSettings() {
    ...
    let errors = Load.validateUserSettings(settings);
    if (errors.length) {
